...
def get_patient(patient_id: str, db: Database = Depends(get_db)):
    data = get_patient_data(db, patient_id)
    if not data:
...
def get_patient_history(patient_id: str, db: Database = Depends(get_db)):
